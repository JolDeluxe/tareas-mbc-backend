...
export const obtenerMisTareas = safeAsync(async (req: Request, res: Response) => {
  const user = req.user!;
  
  // Validar filtros opcionales (solo estatus)
  ...
  const where = {
    ...(estatus && { estatus }),
    responsables: { some: { usuarioId: user.id } }, // Filtro forzado
  };
